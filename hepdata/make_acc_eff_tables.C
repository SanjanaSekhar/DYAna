...
int fill_acc_eff(TTree *t_gen, TH3F *h_dy, TH3F *h_dy_pt, TH3F *h_dy_eta, TH3F *h_dy_all, bool flag_muon){
    ...
    for (int i=0; i<t_gen->GetEntries(); i++) {
        t_gen->GetEntry(i);
            ...
            if (flag_muon){
                ...
                if((eta_p < 1.442 and eta_m < 1.442) or ((eta_p > 1.566 and eta_p < 2.5) and (eta_m > 1.566 and eta_m < 2.5)) )
                    h_dy_eta->Fill(m, rap, gen_cost, evt_weight);
            }
            bool pass = leading > 40 and subleading > 15;
            if(flag_muon) pass = pass and eta_p < 2.4 and eta_m < 2.4 and ang < (TMath::Pi() - 0.005);
            else pass = pass and (eta_p < 1.442 and eta_m < 1.442) or ((eta_p > 1.566 and eta_p < 2.5) and (eta_m > 1.566 and eta_m < 2.5));
...
void make_acc_eff_tables(){
    ...
    TH1F *h16_mumu_dy, *h17_mumu_dy, *h18_mumu_dy, *h16_elel_dy, *h16_elel_dy, *h16_elel_dy, *h16_elel_dy;
    TH1F *h16_mumu_dy_pt, *h17_mumu_dy_pt, *h18_mumu_dy_pt, *h16_elel_dy_pt, *h16_elel_dy_pt, *h16_elel_dy_pt, *h16_elel_dy_pt;
    TH1F *h16_mumu_dy_eta, *h17_mumu_dy_eta, *h18_mumu_dy_eta, *h16_elel_dy_eta, *h16_elel_dy_eta, *h16_elel_dy_eta, *h16_elel_dy_eta;
    ...
    for(int year = 16; year <= 18; year++){
        ...
        fill_acc_eff(t_gen_mu, h_mumu_dy, h_mumu_dy_pt, h_mumu_dy_eta, h_mumu_dy_all, true);
        fill_acc_eff(t_gen_el, h_elel_dy, h_elel_dy_pt, h_elel_dy_eta, h_elel_dy_all, false);
        ...
        FILE *fout_mumu;
	    sprintf(outfile1,"mumu_acc_eff_y%i",year);
	    fout_mumu = fopen(outfile1, "w");
        FILE *fout_elel;
	    sprintf(outfile2,"elel_acc_eff_y%i",year);
	    fout_elel = fopen(outfile2, "w");
        

        for(int i = 1; i <= h1_mumu_dy->GetNbinsX(); i++){

            fprintf(fout_mumu,"%i %.2f %.2f %.2f %.2f", i, h1_mumu_dy->GetBinContent(i), 
                   h1_mumu_dy_pt->GetBinContent(i), h1_mumu_dy_eta->GetBinContent(i), h1_mumu_dy_all->GetBinContent(i));
            fprintf(fout_elel,"%i %.2f %.2f %.2f %.2f", i, h1_elel_dy->GetBinContent(i), 
                   h1_elel_dy_pt->GetBinContent(i), h1_elel_dy_eta->GetBinContent(i), h1_elel_dy_all->GetBinContent(i));
        }
