...
//use events twice and make denom symmetric only
 ...
 //   }
	...
	float get_LQ_scalar_num(float gen_cost,float s,float Q_q, float caq, float cvq, float m_LQ, bool interference, bool negcos, bool test_sign=false){
//	  if (test_sign) cal = crl - cll;
...
int gen_mc_SM_template(TTree *t1, TH3F* h_sym, TH3F *h_asym, TH3F *h_alpha, 
	int year, int flag1 = FLAG_MUONS, bool use_xF = false, const string &sys_label = "" ){

	printf("Making SM mc template for sys %s \n", sys_label.c_str());
	...
	int gen_mc_LQ_template(TTree *t1, TH3F *h_LQpure_u, TH3F *h_LQint_u,TH3F *h_LQpure_d, TH3F *h_LQint_d,TH3F *h_LQpure_u_vec, TH3F *h_LQint_u_vec,TH3F *h_LQpure_d_vec, TH3F *h_LQint_d_vec,
		int year, float m_LQ, int flag1 = FLAG_MUONS, bool make_ud = true, bool use_xF = false, const string &sys_label = "" ){
		...
		for (int i=0; i<tm.nEntries; i++) {
			...
			if(pass){
			 ...
			 if(flag_q!=0){ 
				...
				// vector
				float reweight_LQpure_pos_vec = get_LQ_vec_num(gen_cost, s, Q_q, caq, cvq, m_LQ, false, false)/LQ_denom;
				
				float reweight_LQpure_neg_vec = get_LQ_vec_num(gen_cost, s, Q_q, caq, cvq, m_LQ, false, true)/LQ_denom;

				float reweight_LQint_pos_vec = get_LQ_vec_num(gen_cost, s, Q_q, caq, cvq, m_LQ, true, false)/LQ_denom;
				
				float reweight_LQint_neg_vec = get_LQ_vec_num(gen_cost, s, Q_q, caq, cvq, m_LQ, true, true)/LQ_denom;
		...
		//    sum_weights+=gen_weight;
				...
				if(flag_q!=0){ 
					...
					if(flag_q==1){
						...
						h_LQpure_d->Fill(m, rap, -gen_cost, reweight_LQpure_neg * evt_weight*1e3 );
						h_LQint_d->Fill(m, rap, gen_cost, reweight_LQint_pos * evt_weight *1e3); 
						h_LQint_d->Fill(m, rap, -gen_cost, reweight_LQint_neg * evt_weight*1e3);
					}
							//uLQ temps
					if(flag_q==2){
						h_LQpure_u->Fill(m, rap, gen_cost, reweight_LQpure_pos * evt_weight *1e3); 
						h_LQpure_u->Fill(m, rap, -gen_cost, reweight_LQpure_neg * evt_weight *1e3);
						h_LQint_u->Fill(m, rap, gen_cost, reweight_LQint_pos * evt_weight *1e3); 
						h_LQint_u->Fill(m, rap, -gen_cost, reweight_LQint_neg * evt_weight*1e3);
					}
				}
			}        
		}
		printf("selected %i events; total evt_weight = %f \n", nEvents,sum_weights*1e3);
