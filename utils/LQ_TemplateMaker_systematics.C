...
//also not where xf and cost are defined
using namespace std;
...
int gen_data_template(TTree *t1, TH3F* h,  
    ...
    int nEvents = 0;
    int n=0;
    TempMaker tm(t1, true, year); //templatemaker from tree
...
int gen_mc_template(TTree *t1, Double_t alpha_denom, TH3F* h_sym, TH3F *h_asym, TH3F *h_alpha,
    ...
    for (int i=0; i<tm.nEntries; i++) {
        ...
        if(pass){
            ...
            float var1 = abs(tm.cm.Rapidity());//testing with rapidity instead of mass bins atm
            if(use_xF)  var1 = tm.xF;

            h_sym->Fill(var1, tm.cost, reweight_s * tm.evt_weight); 
            h_sym->Fill(var1, -tm.cost, reweight_s * tm.evt_weight); 

            //h_sym->Fill(var1, tm.cost, tm.m, reweight*evt_wt)
...
int gen_mc_template(TTree *t1, Double_t alpha_denom, TH2F* h_sym, TH2F *h_asym, TH2F *h_alpha,
    ...
    for (int i=0; i<tm.nEntries; i++) {
        ...
        if(pass){
            ...
            h_sym->Fill(var1, tm.cost, reweight_s * tm.evt_weight); 
            h_sym->Fill(var1, -tm.cost, reweight_s * tm.evt_weight); 
//h_sym->Fill(var1,tm.cost,mass?, evt_wt)
            h_asym->Fill(var1, tm.cost, reweight_a * tm.evt_weight);
            h_asym->Fill(var1, -tm.cost, -reweight_a * tm.evt_weight);
