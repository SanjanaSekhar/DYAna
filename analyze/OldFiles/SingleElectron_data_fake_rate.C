...
void SingleElectron_data_fake_rate()
{
    ...
    TH2D *h_pass_HLT = new TH2D("h_pass_HLT", "Rate of passing ISO cut for single electrons with HLT",  n_eta_bins, eta_bins, n_pt_bins, pt_bins);
    TH2D *h_total_HLT = new TH2D("h_total_HLT", "Total number of single electrons with HLT",  n_eta_bins, eta_bins, n_pt_bins, pt_bins);

    TH2D *h_pass_noHLT = new TH2D("h_pass_noHLT", "Rate of passing ISO cut for single electrons with no HLT",  n_eta_bins, eta_bins, n_pt_bins, pt_bins);
    TH2D *h_total_noHLT = new TH2D("h_total_noHLT", "Total number of single electrons with no HLT",  n_eta_bins, eta_bins, n_pt_bins, pt_bins);
            ...
            if( el_size >= 1 && el_IDMedium_NoIso[0] && el_Pt[0] > 29. && abs(el_Eta[0]) < 2.4 &&
                ...
                if(no_other_els && no_bjets && met_pt < 50){
                    if(good_trigger){
                        if(iso_0){
                            nPass++;
                            h_pass_HLT->Fill(abs(el_Eta[0]), el_Pt[0], 1);
                        }
                        h_total_HLT->Fill(abs(el_Eta[0]), el_Pt[0], 1);
                    }
                    else{
                        if(iso_0){
                            nPass++;
                            h_pass_noHLT->Fill(abs(el_Eta[0]), el_Pt[0], 1);
                        }
                        h_total_noHLT->Fill(abs(el_Eta[0]), el_Pt[0], 1);
