...
void convert_emu_qcd_to_param_hist(TH2F *h, FILE *f_log){
    ...
    RooParametricHist *p= new RooParametricHist (h_name, h_name, *var_ss, *bin_list, *h1);
    char norm_name[40];
...
void make_emu_qcd_templates(int year, FILE *f_log){
    ...
    convert_emu_qcd_to_param_hist(h_emu_qcd, f_log); 
...
void make_emu_mc_templates(int year){
    ...
    gen_emu_template(t_emu_back, h_emu_bk,  false, year, m_low, m_high, ss);
    gen_emu_template(t_emu_dy, h_emu_dy,  false, year, m_low, m_high, ss);
    ...
    write_roo_hist(h1_emu_dy, var_ss);
    printf("Made emu dy templates \n");

    write_roo_hist(h1_emu_bk, var_ss);
