...
//    for(int year=2016;year<=2018;year++){
        ...
        int year = 2017;
        char fout_name[200];
        sprintf(fout_name,"combine/templates/LQm%i_R2RLde_gen_nojets_templates%i_060622.root",int(m_LQ),year%2000);
    //    sprintf(fout_name,"combine/templates/LQm%i_SM_gen_nojets_templates%i_041222.root",int(m_LQ),year%2000);
        ...
        char lhe_name[200];
        sprintf(lhe_name,"../analyze/root_files/LQde_m%i_MGevents_ylq1_nojets_060622.root",int(m_LQ));
        ...
        float xsec = 0.1028, xsec_SM = 0.06626;
        ...
        TH1F* h1_LQpure_test = new TH1F(title, "LQ test template", 25,-1.,1.);
        h1_LQpure_test->SetDirectory(0);
        sprintf(title, "ee%i_LQint_test", year %2000);
        TH1F* h1_LQint_test = new TH1F(title, "LQ test template",25, -1.,1.);
    ...
    //nEvents += make_gen_temps(t_gen_mu, h_uncut, h_raw, h_sym, h_asym, h_alpha, m_low, m_high, do_ptrw, year, sys);
        sum_weights = make_gen_temps(t_gen_el, h_raw, h_sym, h_asym, h_alpha, h_LQpure_u, h_LQpure_d, h_LQint_u, h_LQint_d,  m_LQ, year);
	...
	printf("h_LQpure_d %.2f h_LQint_d %.2f \n", h_LQpure_u->Integral(), h_LQint_u->Integral());
  ...
  /*      
	test_sign = true;
        ...
        leg5->Draw();
        sprintf(title, "../generator_stuff/plots/LQpure_u_test_sign_flipcal.png");
        c_mumu5->Print(title);
        delete c_mumu5;
...
*/
       ...
       // h1_data->Scale(.5);
        ...
        c_mumu2->cd(2);
        h1_LQpure_d->SetLineColor(kRed);
        h1_LQint_d->SetLineColor(kGreen+2);
        h1_LQpure_d->SetLineWidth(2);
        h1_LQint_d->SetLineWidth(2);
        h1_LQpure_d->Draw("hist");
        h1_LQint_d->Draw("hist same ");
        sprintf(title, "../generator_stuff/plots/SM_LQd_templates_%i.png", year %2000);
        c_mumu2->Print(title);
        delete c_mumu2;
        fout->Close();
