...
#do
    ...
    combine -M FitDiagnostics  --robustFit 1 -d higgsCombine_nom.MultiDimFit.mH120.root -w w --snapshotName "MultiDimFit" -n _nom1 #&> logs/nomfit_${idx}
    cp impacts/mbin${idx}.json impacts/group_mbin${idx}.json
    for par in ${vars[*]};  do
        combine -M FitDiagnostics --freezeNuisanceGroups $par  --robustFit 1 -d higgsCombine_nom.MultiDimFit.mH120.root -w w --snapshotName "MultiDimFit" -n _${par} #&> logs/${par}fit_${idx}
