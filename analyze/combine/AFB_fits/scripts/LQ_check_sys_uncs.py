	...
	for chan in ["mumu","ee"]:
		for q in ["u","d"]:
			...
			for options.mLQ in [1000,1500,2000,2500,3000,3500]:
				...
				plt.errorbar(df_all['Sys name'],df_all['Mean'],yerr=[-df_all["Std"],df_all["Std"]], label="mLQ=%i GeV"%(options.mLQ))
			plt.legend()
	...
	for options.mLQ in [1000,1500,2000,2500,3000,3500]:
		for chan in ["mumu","ee"]:
			...
			for q in ["u","d"]:
				...
				plt.errorbar(df_all['Sys name'],df_all['Mean'], yerr=[-df_all["Std"],df_all["Std"]], label="%s-%s%s"%(chan,q,("-vec" if is_vec else "")))
			plt.legend()
