...
def plotLimits(channel):
 ...
 # Create an empty TH1 from the first TGraph to serve as the pad axis and frame
    ...
    if is_vec:
	if 'm' in channel: 
		axis.GetXaxis().SetTitle(r'$m_{V_{\mu %s}}$ (GeV)'%(channel[0]))
		axis.GetYaxis().SetTitle(r'Limits on $g_{\mu %s}$'%(channel[0]))
	else: 
		axis.GetXaxis().SetTitle(r'$m_{V_{e %s}}$ (GeV)'%(channel[0]))
		axis.GetYaxis().SetTitle(r'Limits on $g_{e %s}$'%(channel[0]))
    else:
	if 'm' in channel: 
		axis.GetXaxis().SetTitle(r'$m_{S_{\mu %s}}$ (GeV)'%(channel[0]))
		axis.GetYaxis().SetTitle(r'Limits on $y_{\mu %s}$'%(channel[0]))
        else: 
		axis.GetXaxis().SetTitle(r'$m_{S_{e %s}}$ (GeV)'%(channel[0]))
		axis.GetYaxis().SetTitle(r'Limits on $y_{e %s}$'%(channel[0]))
    pads[0].cd()
    axis.Draw('axis')
