...
#            options.q="u"
            ...
            #print(" \n \n Starting fit for bin %i \n\n" % mbin)
                ...
                print_and_do("combine %s -M MultiDimFit  --saveWorkspace --saveFitResult --robustFit 1 %s --freezeParameters Afb,A0 -v 3 " %(workspace, extra_params))
              #  print_and_do("combine %s -M MultiDimFit --saveWorkspace --saveFitResult --robustFit 1  %s " %(workspace, extra_params))
                ...
                if(not options.no_plot):
                    ...
                    print_and_do("combine %s -M FitDiagnostics --skipBOnlyFit %s --freezeParameters Afb,A0 -v 3" % (workspace, extra_params)) #only to get prefit, probably a better way
