...
print_and_do("combine -M FitDiagnostics -d %s --saveWorkspace --robustFit 1" % (workspace))
setSnapshot(Afb_val = -1.)
...
c1.Print("%sbias_test_mbin%i_afb%.0f.png" %(options.mbin, 100.* options.Afb))
...
c2.Print("%sbias_test_mbin%i_Az%.0f.png" %(options.mbin, 100.* options.A0))
