...
for is_vec in [True]:
	for chan in ["ee"]:
		for q in ["d"]:
			...
			#print_and_do("combine -M FitDiagnostics -d %s -t -1 --setParameters yLQ2=0.0,A0=0.05,A4=1.6  --forceRecreateNLL -n _t0" %workspace)
			...
			'''
			print_and_do("combineTool.py -M Impacts -d %s -t -1 --setParameters yLQ2=0.0,A0=0.05,A4=1.6 -m 2000 --doInitialFit --allPars -n t0"%workspace)
			print_and_do("combineTool.py -M Impacts -d %s -t -1 --setParameters yLQ2=1.0,A0=0.05,A4=1.6 -m 2000 --doInitialFit --allPars -n t1"%(workspace))

			print_and_do("combineTool.py -M Impacts -d %s -o impacts_t0_%s_%s_%s.json -t -1 --setParameters yLQ2=0.0,A0=0.05,A4=1.6 --doFits -m 2000 -n t0 "%(workspace,chan,q,("vec" if is_vec else "")))
			
			print_and_do("combineTool.py -M Impacts -d %s -o impacts_t1_%s_%s_%s.json -t -1 --setParameters yLQ2=1.0,A0=0.05,A4=1.6 --doFits -m 2000 -n t1 "%(workspace,chan,q,("vec" if is_vec else "")))
