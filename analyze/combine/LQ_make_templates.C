...
void make_data_templates(int year, bool scramble_data = true, bool fake_data = true){
    ...
    if(!fake_data){
        ...
        TTree *mu_ts[7] = {t_mumu_mc, t_mumu_tautau, t_mumu_ttbar, t_mumu_diboson, t_mumu_wt, t_mumu_gamgam};
        TTree *el_ts[7] = {t_elel_mc, t_elel_tautau, t_elel_ttbar, t_elel_diboson, t_elel_wt, t_elel_gamgam};
...
void LQ_make_templates(int year = 2016, int nJobs = 6, int iJob =-1){
   ...
   // for(int i=6;i<=10;i++){
    ...
    sprintf(templates_name,"combine/templates/LQm%i_3rap_templates%i.root",int(m_LQ),year%2000);
