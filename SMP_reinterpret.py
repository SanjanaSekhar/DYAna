...
def LQ_cost(flag, cost, s, m_lq, y_lq, prnt = False):
        ...
        D = D1 + D2
        if flag==1: C, D = C/3., D/3. # d quark PDF
        else: C, D = 2.*C/3. , 2.*D/3. # u quark PDF
...
y_lq_list = np.linspace(0.01,2.,100)
for chan in ['mumu']:
        ...
        for flag in [2]:
                ...
                for mlq in m_lq_list:
                        ...
                        if chan=="ee":
                                plt.axhline(y=0.694,color='r',label=r'$A_{FB}$ in mass bin > 1 TeV from SMP-21-002')
                                # plt.axhline(y=0.6911,color='g')
                                # plt.axhline(y=0.7442,color='b')
                                plt.fill_between(np.linspace(-0.5,2.),0.542,0.618,color='yellow',label=r'$\pm 2\sigma$')
                                plt.fill_between(np.linspace(-0.5,2.),0.77,0.846,color='yellow')
                                plt.fill_between(np.linspace(-0.5,2.),0.694,0.77,color='chartreuse',label=r'$\pm 1\sigma$')
                                plt.fill_between(np.linspace(-0.5,2.),0.618,0.694,color='chartreuse')
                                plt.plot(ylq,B,color='black',label=r'$A_{FB}$ extracted from analytical fit to LQ signal')
                                s = [0.542]#,0.618,0.694,0.77,0.846]
                                for sig in s:
                                ...
                                plt.axhline(y=0.595,color='r',label=r'$A_{FB}$ in mass bin > 1 TeV from SMP-21-002')
                                # plt.axhline(y=0.6911,color='g')
                                # plt.axhline(y=0.7442,color='b')
                                plt.fill_between(np.linspace(-0.5,2.),0.455,0.525,color='yellow',label=r'$\pm 2\sigma$')
                                plt.fill_between(np.linspace(-0.5,2.),0.665,0.735,color='yellow')
                                plt.fill_between(np.linspace(-0.5,2.),0.595,0.665,color='chartreuse',label=r'$\pm 1\sigma$')
                                plt.fill_between(np.linspace(-0.5,2.),0.525,0.595,color='chartreuse')
                                plt.plot(ylq,B,color='black',label=r'$A_{FB}$ extracted from analytical fit to LQ signal')
                                s = [0.455]#,0.525,0.595,0.665,0.735]
                                for sig in s:
                                        diff, ylq_diff = B[B - sig < 0.0001], ylq[B - sig < 0.0001]
                                        diff, ylq_diff = diff[sig - diff > 0.0001], ylq_diff[sig - diff > 0.0001]
                                        afb_limit.append(ylq_diff[0])
                ...
                plt.plot(m_lq_list, afb_limit)
                #plt.xlim(0.,1.7)
                plt.ylim(0.,1.7)
                plt.xlabel(r"$M_{S_{%s%s}}$"%(chan[0],"u" if flag==2 else "d"))
                plt.ylabel(r"$y_{S_{%s%s}}$"%(chan[0],"u" if flag==2 else "d"))
                plt.title(r"$S_{%s%s}$ Limits recast from SMP-21-002"%(chan[0],"u" if flag==2 else "d"))
