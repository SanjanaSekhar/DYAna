...
void draw_rap_cmp(){
    ...
    for(int i=0; i<n_m_bins; i++){
        ...
        make_m_cost_pt_xf_hist(t_elel_data, dummy, dummy, dummy, dummy, dummy, data_rap, true, type,  year, m_low, m_high);
        make_m_cost_pt_xf_hist(t_elel_mc, dummy, dummy, dummy, dummy, dummy,  dy_rap,              false, type,   year, m_low, m_high);
        make_m_cost_pt_xf_hist(t_elel_tautau, dummy, dummy, dummy, dummy, dummy,  dy_tautau_rap, false, type,  year, m_low, m_high);
        make_m_cost_pt_xf_hist(t_elel_ttbar, dummy, dummy, dummy, dummy, dummy, ttbar_rap, false, type,  year, m_low, m_high);
        make_m_cost_pt_xf_hist(t_elel_wt, dummy, dummy, dummy, dummy, dummy, wt_rap, false, type,  year, m_low, m_high);
        make_m_cost_pt_xf_hist(t_elel_gamgam, dummy, dummy, dummy, dummy, dummy, gg_rap, false, type,  year, m_low, m_high);
        make_m_cost_pt_xf_hist(t_elel_diboson, dummy, dummy, dummy, dummy, dummy, diboson_rap, false, type,   year, m_low, m_high);



        bool ss_qcd = false;
        make_fakerate_est(t_elel_WJets, t_elel_QCD, t_elel_WJets_contam, t_elel_QCD_contam, dummy, dummy, dummy, dummy, dummy, QCD_rap, type, year, m_low, m_high, ss_qcd);
        ...
        char leg_title[100];
        sprintf(leg_title, "Electrons %i, Mass %.0f - %.0f GeV", year, m_low, m_high);
        ...
        leg1->AddEntry(gg_rap, "#gamma#gamma #rightarrow ee", "f");
