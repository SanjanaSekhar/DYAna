...
void make_m_cost_xf_hist(TTree *t1, TH1F *h_cost_st, TH1F *h_cost, TH1F *h_xF, bool is_data=false){
    ...
    for (int i=0; i<size; i++) {
        ...
        if(!is_tau_event && m >= 150 && met_pt < 50. && no_bjets){
            if(is_data){
                ...
                h_cost_st->Fill(cost_st,evt_weight);
    ...
    h_cost_st->Scale(1000*(bcdef_lumi + gh_lumi));
    h_cost->Scale(1000*(bcdef_lumi + gh_lumi));
    h_xF->Scale(1000*(bcdef_lumi + gh_lumi));
    t1->ResetBranchAddresses();
}

void draw_cost_xf(){
    setTDRStyle();
    TFile *f_mc = TFile::Open("../analyze/output_files/ElEl_DY_aug24.root");
    TTree *t_mc = (TTree *)f_mc->Get("T_data");
    TTree *t_mc_nosig = (TTree *)f_mc->Get("T_back");
    int n_m_bins = 6; 
    Double_t m_bins[] = {150,200,250,350,500,700,1000}; 

    TH1F *h_xF_mc = new TH1F("h_xF_mc", "e^{+}e^{-} Feynman-x; |xF|", 20, 0, 0.5);
    ...
    TH1F *h_xF_mc_nosig = new TH1F("h_xF_mc_nosig", "e^{+}e^{-} Feynman-x; |xF|", 20, 0, 0.5);
    ...
    //h_cost_mc->Print();
    h_cost_mc->Draw("hist");
    h_cost_mc->GetXaxis()->SetTitle("DY to ee Colins-Soper Cos(#theta)");
