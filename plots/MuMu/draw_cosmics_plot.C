    ...
    //read event data
        ...
        for (int i=0; i<tm.nEntries; i++) {
            ...
            if(pass){
                ...
                h_eta->Fill(tm.lep_p->Eta() + tm.lep_m->Eta(), tm.evt_weight);

                if(!tm.not_cosmic){ //fails cosmic cut
                    cosmic_sum += tm.evt_weight;
                }
            }
            
        
        }
        printf("Selected %i entries.\n", nEvents);
        printf("%.0f Events  removed by cosmics cut \n", cosmic_sum);
...
void draw_cosmics_plot(){
    ...
    float dphi_start = TMath::Pi() - 1.0;
    TH1F *data_dphi = new TH1F("data_dphi", "Data Dimuon Mass Distribution", n_dphi_bins, dphi_start, dphi_max);
    TH1F *mc_dphi = new TH1F("mc_dphi", "MC Signal (qqbar, qglu, qbarglu)", n_dphi_bins, dphi_start, dphi_max);
    TH1F *mc_tautau_dphi = new TH1F("mc_tautau_dphi", "MC no signal (qq, gluglu qbarqbar)", n_dphi_bins, dphi_start, dphi_max);
    TH1F *ttbar_dphi = new TH1F("ttbar_dphi", "TTBar Background", n_dphi_bins, dphi_start, dphi_max);
    TH1F *diboson_dphi = new TH1F("diboson_dphi", "DiBoson (WW, WZ, ZZ)", n_dphi_bins, dphi_start, dphi_max);
    TH1F *gg_dphi = new TH1F("gg_dphi", "QCD", n_dphi_bins, dphi_start, dphi_max);
    TH1F *WJets_dphi = new TH1F("WJets_dphi", "WJets", n_dphi_bins, dphi_start, dphi_max);
    TH1F *wt_dphi = new TH1F("wt_dphi", "tw + #bar{t}w", n_dphi_bins, dphi_start, dphi_max);


    int n_eta_bins = 40;
    float eta_max = 0.25;
    float eta_start = -0.25;
    TH1F *data_eta = new TH1F("data_eta", "Data Dimuon Mass Distribution", n_eta_bins, eta_start, eta_max);
    TH1F *mc_eta = new TH1F("mc_eta", "MC Signal (qqbar, qglu, qbarglu)", n_eta_bins, eta_start, eta_max);
    TH1F *mc_tautau_eta = new TH1F("mc_tautau_eta", "MC no signal (qq, gluglu qbarqbar)", n_eta_bins, eta_start, eta_max);
    TH1F *ttbar_eta = new TH1F("ttbar_eta", "TTBar Background", n_eta_bins, eta_start, eta_max);
    TH1F *diboson_eta = new TH1F("diboson_eta", "DiBoson (WW, WZ, ZZ)", n_eta_bins, eta_start, eta_max);
    TH1F *gg_eta = new TH1F("gg_eta", "QCD", n_eta_bins, eta_start, eta_max);
    TH1F *WJets_eta = new TH1F("WJets_eta", "WJets", n_eta_bins, eta_start, eta_max);
    TH1F *wt_eta = new TH1F("wt_eta", "tw + #bar{t}w", n_eta_bins, eta_start, eta_max);
